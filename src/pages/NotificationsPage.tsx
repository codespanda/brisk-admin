...
import { CheckCircle, AlertTriangle, XCircle, Info, CheckCheck, Trash2, Bell } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
          ...
          {notifications.length === 0 ? (
            ...
            <div>
              {notifications.map((n, i) => {
                ...
                return (
                  <div key={n.id}>
                    <div className={cn('flex items-start gap-4 px-6 py-4 transition-colors', !n.read && 'bg-primary/[0.03]')}>
                      ...
                      <div className="flex shrink-0 items-center gap-1 mt-1">
                        {!n.read && (
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground" onClick={() => markRead(n.id)}>
                            <CheckCheck className="h-4 w-4" />
                          </Button>
                        )}
                        <Button
                          ...
                          onClick={() => { remove(n.id); toast.success('Notification removed') }}
                        >
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
      ...
      <ConfirmDialog
        ...
        variant="destructive"
        onConfirm={() => { removeAll(); setRemoveAllOpen(false); toast.success('All notifications removed') }}
      />
    </div>
  )
}
